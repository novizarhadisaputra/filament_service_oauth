...
namespace App\Filament\Resources\Systems\Resources\OAuthClients\Tables;
        ...
        return $table
            ...
            ->recordActions([
                ...
                Action::make('regenerateSecret')
                    ->action(fn (OAuthClient $record) => $record->update(['secret' => \Illuminate\Support\Str::random(40)]))
                    ->requiresConfirmation()
                    ->color('warning')
                    ->icon('heroicon-o-key')
                    ->visible(fn (OAuthClient $record) => auth()->user()->can('viewAny', $record)),
            ])
            ->toolbarActions([
